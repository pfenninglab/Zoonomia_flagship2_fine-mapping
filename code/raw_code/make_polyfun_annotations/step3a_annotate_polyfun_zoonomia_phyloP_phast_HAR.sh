...
FILES=$(ls $DATADIR/bed_hg19/*.hg19.bed.gz )
...
PARQUET=$(ls ${ANNOTDIR}/zoo.*.${SLURM_ARRAY_TASK_ID}.annot.parquet | sed '/ENCODE3/d' | tr '\n' ','| sed 's/,$//g')
...
sbatch --partition=pfen1 --time 23:00:00 --job-name=${LABEL} --mem=45G --array=${SLURM_ARRAY_TASK_ID} --time 1-0:00:00 \
