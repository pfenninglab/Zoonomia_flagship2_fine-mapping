...
if [[ ! -f ${DATADIR}/${PREFIX}/${PREFIX}_Zoonomia_conservation_aggregate.txt.gz && -f ${OUTDIR}/polyfun_all_jobs_22.txt ]]; then
...
if [[ ! -f ${DATADIR}/${PREFIX}/${PREFIX}_Zoonomia_conservation_aggregate.txt.gz && -f ${OUTDIR}/polyfun_all_jobs_22.txt ]]; then
echo "Submitting job for ${PREFIX} GWAS."; 
sbatch -p pfen_bigmem,pool3-bigmem,pfen3 --mem 150G --time 4-4:00:00 --export=JobsFileName=${JobsFileName} \
${SETWD}/code/raw_code/nonfunct_finemapping/slurm_finemap_byLine.sh
fi; done
...
OUTDIR=${DATADIR}/${PREFIX}/susie
# if [[ ! -f ${DATADIR}/${PREFIX}/${PREFIX}_Zoonomia_conservation_top_annot.txt.gz && -f ${OUTDIR}/polyfun_all_jobs_22.txt ]]; then
echo "Aggregating results from ${PREFIX} GWAS with P < ${CUTOFF} cutoff for loci."
sbatch --export=OUTDIR=${OUTDIR},PREFIX=${PREFIX}_Zoonomia_conservation,SUMSTATS=${SUMSTATS},CUTOFF=${CUTOFF} \
